...
def make_parser():
    ...
    parser.add_argument(
        "--upgrade-code",
        type=str,
        default="62eb3032-f9fb-4440-8def-b9d7bdffd9f4",
...
def gen_auto_component(app_name, dist_dir):
    return gen_content_between_tags(
        "picguard/Components/PicGuard.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def gen_pre_vars(args, dist_dir, icon_path):
    ...
    return gen_content_between_tags(
        "picguard/Includes.wxi", "<!--$PreVarsStart$-->", "<!--$PreVarsEnd$-->", func
    )
...
def gen_upgrade_info():
    ...
    return gen_content_between_tags(
        "picguard/Fragments/Upgrades.wxs",
        "<!--$UpgradeStart$-->",
        "<!--$UpgradeEnd$-->",
        func,
    )
...
def gen_custom_dialog_bitmaps():
    def func(lines, index_start):
        ...
        to_insert_lines = []
        for var in vars:
            if Path(f"picguard/Resources/{var}.bmp").exists():
...
def gen_custom_ARPSYSTEMCOMPONENT_False(args):
    ...
    return gen_content_between_tags(
        "picguard/Fragments/AddRemoveProperties.wxs",
        "<!--$ArpStart$-->",
        "<!--$ArpEnd$-->",
        func,
    )
...
def replace_component_guids_in_wxs():
    langs_dir = Path(sys.argv[0]).parent.joinpath("picguard")
