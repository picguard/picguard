...
if [ ! -f "$FLUTTER_DIR/flutter_linux_3.35.2-stable.tar.xz" ]; then
   echo "🟩 Downloading Flutter"
   wget https://storage.googleapis.com/flutter_infra_release/releases/stable/linux/flutter_linux_3.35.2-stable.tar.xz -P "$FLUTTER_DIR"
fi
...
echo "🟩 Unzipping"
mkdir "$FLUTTER_DIR/flutter" && tar -xf "$FLUTTER_DIR/flutter_linux_3.35.2-stable.tar.xz" -C "$FLUTTER_DIR/flutter" --strip-components 1
